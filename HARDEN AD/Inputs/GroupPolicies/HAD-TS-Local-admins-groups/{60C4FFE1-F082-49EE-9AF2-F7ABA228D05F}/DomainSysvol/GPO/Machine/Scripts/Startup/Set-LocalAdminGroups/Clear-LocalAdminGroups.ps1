...
$GroupNameFilter = "$($myConfig.translation.'PREFIX-DOMLOC')$(($myConfig.translation.'GRP-NAME').REPLACE('%ComputerName%',''))"
...
foreach ($group in $Groups)
{
    ...
    {
        Try {
            $void = Remove-ADGroup -Identity $group.NAME -Server $myPDC -Confirm:$false
